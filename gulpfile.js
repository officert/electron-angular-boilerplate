...
var VENDOR_JS = [
  // BUILD_DIR + '/app/browser/vendor/jquery/dist/jquery.js',
  BUILD_DIR + '/app/browser/vendor/bootstrap/dist/js/bootstrap.js',
  BUILD_DIR + '/app/browser/vendor/console-polyfill/index.js',
  BUILD_DIR + '/app/browser/vendor/angular/angular.js',
  BUILD_DIR + '/app/browser/vendor/angular-resource/angular-resource.js',
  BUILD_DIR + '/app/browser/vendor/angular-ui-router/release/angular-ui-router.js',
  BUILD_DIR + '/app/browser/vendor/angular-bootstrap/ui-bootstrap-tpls.js',
  BUILD_DIR + '/app/browser/vendor/underscore/underscore.js',
  BUILD_DIR + '/app/browser/vendor/angular-animate/angular-animate.js',
  BUILD_DIR + '/app/browser/vendor/angular-sanitize/angular-sanitize.js',
  BUILD_DIR + '/app/browser/vendor/angular-scroll/angular-scroll.js',
  BUILD_DIR + '/app/browser/vendor/moment/moment.js'
];
...
gulp.task('copy', ['clean', 'copy-bower'], function(next) {
  return _init(gulp.src(['package.json', 'src/**/*.*', 'src/bower_components/**/*.css', 'src/bower_components/**/*.less', 'src/bower_components/**/*.html', '!src/bower_components/**/*.js']))
    .pipe(gulp.dest(BUILD_DIR));
});

gulp.task('copy-bower', ['clean'], function(next) {
  return _init(gulp.src('src/app/broswer/vendor/**/**/*.js'))
    .pipe(wrap('(function(){<%= contents %>\n})();'))
    .pipe(gulp.dest(BUILD_DIR + '/src/app/broswer/vendor'));
});
...
gulp.task('replace', ['copy'], function() {
  return _replace(gulp.src(['build/**/**/*', '!build/app/broswer/vendor/**/*.js']))
    .pipe(gulp.dest(BUILD_DIR));
});
...
gulp.task('css', ['js', 'css-vendor'], function() {
  var target = gulp.src(BUILD_DIR + '/app/browser/index.html');

  var sources = gulp.src(BUILD_DIR + '/app/browser/less/main.less')
    .pipe(less(LESSOPTIONS))
    .pipe(rename(MINIFIED_SRC_CSS))
    .pipe(gulp.dest(BUILD_DIR + '/app/browser/css'));

  return target.pipe(inject(sources, {
      starttag: '<!-- injectSrcCss:css -->',
      endtag: '<!-- endinjectSrcCss -->',
      transform: function(filepath) {
        var path = filepath.replace('/build/app/browser/', '');
        return '<link rel="stylesheet" href="' + path + '"/>';
      }
    }))
    .pipe(gulp.dest(BUILD_DIR + '/app/browser'));
});
...
gulp.task('js', ['js-vendor', 'copy', 'replace'], function() {
  ...
  var sources = gulp.src([BUILD_DIR + '/app/browser/**/*.js', '!' + BUILD_DIR + '/app/browser/vendor/**/*.js', '!' + BUILD_DIR + '/app/browser/' + MINIFIED_VENDOR_SCRIPT])
    .pipe(gulpIf(ENV_PROD, uglify(MINIFIED_SRC_SCRIPT, UGLIFYOPTIONS)))
    .pipe(gulpIf(ENV_PROD, gulp.dest(BUILD_DIR + '/app/browser')));
...
gulp.task('jshint', ['replace'], function() {
  return _init(gulp.src(['src/app/browser/**/*.js']))
    .pipe(jshint())
